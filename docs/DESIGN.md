...
### 14.15 Four dashboard table panels silently broken by a Grafana transform that doesn't do what its name suggests

Found 2026-08-25 during Stage 16's SCC soak-test session, while checking the two panels CLAUDE.md had flagged as unconfirmed ("Interface inventory", "Exporter build info") in a genuine non-headless browser session rather than the scripted-scroll capture that first surfaced them. Both reproduced as real bugs, not headless-capture artifacts — and tracing the mechanism found the same defect in two more panels, "Per-device summary" (the dashboard's flagship panel per §10.2) and "Current HA role", plus a fifth ("S2S tunnels not up") caught only by an adversarial review of the fix, not live reproduction (the test fleet has no S2S VPN configured, so it rendered as merely empty rather than visibly broken).

**Root cause, confirmed live against a real Grafana 13.1.3 (via its `/api/ds/query` API and panel data inspector, not guessed):** a Prometheus instant query returns one series per label combination, and each series' labels arrive as **field metadata**, not as separate table columns. `merge` — the transform all five panels used to combine multiple such series into one table — concatenates frames that already share an identical *field-name signature*; it never promotes label metadata to fields. Since each series' value field is named after its own metric (or its `legendFormat`, depending on whether `labelsToFields` ran first), two different metrics' series never share a signature, and `merge` alone renders either a single degenerate row (one target) or several disjoint row-groups, one per target, each showing dashes for every other target's columns (multiple targets) — while the `organize` step's `renameByName`, keyed on label names that were never promoted to fields, silently renames nothing.

**The fix, per panel shape:**
- Single target, single row's worth of labels to expose ("Exporter build info"): `labelsToFields` alone.
- Single target, many series needing to become many rows ("S2S tunnels not up"): `labelsToFields` then `merge` (`merge` correctly concatenates once the frames are already label-promoted and share a signature).
- Multiple targets whose series carry an **identical** label schema apart from `__name__` ("Interface inventory" — `ftd_interface_operational_up`/`ftd_interface_link_up` share every other label): `joinByLabels` with `value: '__name__'`, which pivots each metric into its own column and joins on every other shared label in one step.
- Multiple targets whose series carry **different** label schemas ("Current HA role", "Per-device summary"): `joinByLabels` fundamentally cannot reconcile this — every label not listed in its `join` option (and not the `value` pivot) is unconditionally dropped from the output, for every row, not just the mismatched ones, confirmed by testing with a single target and no cross-target conflict at all. The fix normalizes at the **PromQL level**, before Grafana's transform pipeline ever sees it: `group_left` to attach one metric's exclusive label onto the other's series ("Current HA role": `(ftd_ha_node_info{...}==1) * on(job, instance, device_uid, device_name) group_left(status) (ftd_ha_node_status{...}==1)`), and `max by(__name__, device_uid, device_name)(...)` to drop a label one target has and another never did while deliberately keeping `__name__` (aggregations drop it unless it's named in the `by` list). "Per-device summary" additionally **dropped** its HA status/role columns entirely rather than force a join across the same asymmetric-schema problem a second time — "Current HA role" is now a correctly-working dedicated panel for that data, making the duplication unnecessary as well as unreconcilable.
- `label_replace`-injecting a synthetic metric name was tried and rejected on both PromQL and test-suite grounds: it isn't a real declared metric (caught by `test/unit/dashboard-and-alerts.test.ts`'s cross-check against `src/metrics/`), and it adds a `label_replace` occurrence with no matching `max by(job, instance, device_uid, device_name, interface_name)(...)` fold, tripping a *different*, unrelated guard in the same file that counts occurrences of each fleet-wide rather than per-construct.

**Two further, more subtle bugs an adversarial review caught in the first pass of this fix, before it shipped:** the `sum by(...)` originally used to normalize "Per-device summary"'s CPU/memory targets would have silently **added together** two scrape targets' values for the same device the moment a second exporter/job covers overlapping fleet — a physically impossible number (e.g. 1.6 for a 0–1 ratio) that would trip alert thresholds without any actual fault. Changed to `max by(...)`, which degrades to "pick one" under that overlap instead of fabricating a value. Separately, "Current HA role"'s original `group_left` matched on `(device_uid, device_name)` alone, omitting `job, instance` — under the same multi-exporter overlap, PromQL's `group_left` requires the right-hand side unique per match group, so two scrape targets reporting one device would make the *entire query* fail outright ("many-to-many matching not allowed"), taking down every device's row, not just the colliding one. Both fixes cost nothing in the single-exporter case this was tested against; both would have been invisible in this session's real 2-device single-job fleet, exactly the kind of thing this project's own alert rules already defend against elsewhere (`alerts/ftd-health.yaml`'s `unless on(job, instance, device_uid, device_name, interface)`).

**Regression coverage added:** `test/unit/dashboard-and-alerts.test.ts` gained a generic structural check — any panel using `merge` must use `labelsToFields` earlier in the same transformation list — plus a negative control proving the check actually fails when that invariant is violated. This guards the root cause, not just the five panels found broken this session; nothing in the existing 733-test suite (metric-name cross-referencing, the label_replace/fold-ratio guard, grid/layout checks) would have caught any of this, before or after the fix, because a syntactically well-formed but semantically inert transform pipeline is invisible to all of them.

All four originally-broken panels (plus the fifth found by review) were re-verified live against the real Grafana/Prometheus instance this session's soak test was already running against, panel-by-panel, after the fix.
